...
Terceiro projeto realizado para o Bootcamp da [Laboratória](https://www.laboratoria.la/br), na turma SAP009. O projeto consiste em uma rede social responsiva destinada a unir mulheres apaixonadas por música, por descobrir novos artistas e acompanhar os lançamentos de novos álbuns musicais. A interface permite a criação de uma conta de cadastro e login através do firebase. As usuárias podem interagir através de postagens e acessam a aplicação através da criação de contas e login autenticados pelo Firebase. 
